...
#figure(
  stack(image("s3/esch/cuneiform-2.svg", width: 80%)),
...
#figure(
  stack(
    spacing: 0.75em,
    $alpha space beta space gamma space delta space epsilon space zeta space eta space theta space iota space kappa space lambda space mu space nu space xi space omicron space pi space rho space sigma space tau space upsilon space phi space chi space psi space omega$,
    $Alpha space Beta space Gamma space Delta space Epsilon space Zeta space Eta space Theta space Iota space Kappa space Lambda space Mu space Nu space Xi space Omicron space Pi space Rho space Sigma space Tau space Upsilon space Phi space Chi space Psi space Omega$,
  ),
...
// the four nucleotides of DNA ${A, C, T, G}$, and the binary alphabet ${0, 1}$.
// Any countable alphabet can be encoded with another.
// The nucleotides could be encoded as ${01, 011, 0111, 01111}$.
// The invention of the written language was a major milestone in human history,
// akin to the domestication of wolves into dogs, the invention of the needle,
// and the mastery of fire. The ubiquity of the symbol is what splits the past into the historic and prehistoric. There are many attributes of the symbo


// It has been said that in our information-theoretic age, the bit has replaced the
// atom as the most fundamental unit of wealth @mcafee2019.
// If any phenomenon befits the name _grand remplacement_, it is this.
// The mid 20-th century saw information formally defined as $ln(1/p_x_i)$,
// where $x_i$ is a symbol from an alphabet $X = {x_0, x_1, ..., x_(n-1)}$ @shannon1948,
// each occurring with probability $P = {p_0, p_1, ..., p_(n-1)}$.
// Consider an unfair coin where $X = {"H", "T"}$ and $P={1, 0}$
// (i.e., we know it will always land on heads).
// In that case, using our formula for information we get $ln(1/1) = 0$.
// We gain no (0) information wathcing the coin flip heads, as we already knew that heads would be the outcome.
// In the fair-coin-case we have $ln(1/0.5) approx 0.693$,
// both the $H$ and $T$ case. Defined as such, the more unlikely or suprising a given event is, the higher its information content.
// Alphabets of arbitrary countable sizes, can be encoded with a two-digit alphabet. In the case of DNA, we have $X = {A, C, T, G}$, which could be encoded as $X' = {00, 01, 10, 11}$ or $X' = {01, 011, 0111, 01111}$

// The notion of information as an exact, measurable quantity was given to us by #cite(<shannon1948>, form: "prose") with his seminal #cite(<shannon1948>, form: "year") paper _A Mathematical Theory of Communication_. With it, he establishes information theory as a science, introducing three towering theorems on which our modern world is (arguably) built: _1)_ The Data Compression Theorem, proving that for a given source that produces symbols from an alphabet $xi = {s_0, s_1, ..., s_(n-1)}$ with probabilities ${p_0, p_1, ..., p_(n-1)}$, there exists an optimal coding scheme that can compress messages to an average length approaching the source entropy; _2)_ The Channel Coding Theorem, proving that reliable communication over noisy channels is possible, at the rate of mutual information between the two endpoints; and _3)_ The Rate-Distortion Theorem, describing the minimum bit rate required to achieve a given level of distortion when compressing beyond the entropy. Information theory, as such, is about the transmission of symbols. The transmission of symbols, however, is about much more than one might think. Indeed, the symbol too reigns supreme in the context of even deoxyribonucleic acid (DNA), a source with the alphabet ${A, C, T, G}$. Senescence is the failure of the body to transmit its own DNA undistorted forward in time to itself. Mortality _is_ thus an information-theoretic phenomenon, and as such, if any, it will have an information theoretic solution @bin-jumah2022.
