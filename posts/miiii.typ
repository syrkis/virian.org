...
#let cover = "https://s3.syrkis.com/esch/anim_grid.svg"
...
#figure(
  image("s3/miiii/polar.svg", width: 100%),
  caption: [
...
#figure(
  image("s3/miiii/x_11_plot.svg", width: 110%),
...
#figure(
  image("s3/miiii/y_11_plot.svg", width: 120%),
...
#figure(
  table(
    ...
    align: center,
    table.header("dropout", $lambda$, "wd", $d$, "lr", "heads"),
    $0, 1 / 2, 1 / 5, 1 / 10$,
    $0, 1 / 2, 2$,
    $0, 1 / 10, 1 / 2, 1$,
    $128, 256$,
...
#figure(
  align(horizon, stack(
    ...
    image("s3/miiii/neurs_113_miiii_one.svg", width: 40%),
    image("s3/miiii/neurs_113_miiii_fft_one.svg", width: 46%),
  )),
...
Additionally, the embedding layers will be inspected using Fourier analysis and singular value decomposition. As mentioned in , our interpretability approach combines activation visualization with frequency analysis to understand the learned algorithmic patterns. Following #cite(<nanda2023>, form: "prose", style: "american-psychological-association"), we analyze both the attention patterns and the learned representations through several lenses:
...
This is a fundamental tool used in signal processing. The theory is briefly outlined in for reference.
...
The default basis of the one-hot encoded representation of the input is thus the identity matrix. This can be projected into a Fourier basis by multiplying with the discrete Fourier transform (DFT) matrix visualized in .
...
#figure(
  stack(
    dir: ttb,
    image("s3/miiii/" + f_hash + "/acc_train_training.svg", width: 85%),
    image("s3/miiii/" + f_hash + "/acc_valid_training.svg", width: 85%),
  ),
...
#figure(
  stack(
    dir: ttb,
    image("s3/miiii/" + f_hash + "/loss_train_training.svg", width: 85%),
    image("s3/miiii/" + f_hash + "/loss_valid_training.svg", width: 85%),
  ),
...
#figure(
  stack(image("s3/miiii/nanda_S.svg"), image("s3/miiii/miiii_S.svg")),
...
#figure(
  stack(dir: ttb, image("s3/miiii/fourier_nanda_m.svg"), image(
    "s3/miiii/fourier_nanda_f.svg",
  )),
...
#figure(
  stack(dir: ttb, image("s3/miiii/fourier_miiii_m.svg"), image(
    "s3/miiii/fourier_miiii_f.svg",
  )),
...
#figure(
  stack(image("s3/miiii/fourier_basis_m.svg"), image(
    "s3/miiii/fourier_basis_f.svg",
  )),
...
#figure(
  stack(dir: ttb, image("s3/miiii/neurs_113_miiii_three.svg"), image(
    "s3/miiii/neurs_113_miiii_fft_three.svg",
  )),
...
#figure(
  stack(dir: ttb, image("s3/miiii/neurs_113_basis.svg"), image(
...
#figure(
  stack(dir: ttb, image("s3/miiii/miiii_large_finding.svg"), image(
    "s3/miiii/miiii_small_finding.svg",
  )),
...
#figure(
  table(
    columns: (1fr, 1fr, 1fr, 1fr, 1fr, 1fr),
    align: center,
    table.header("epoch", "256", "1024", "4096", "16384", "65536"),
    $|omega|$, $0$, $0$, $10$, $18$, "10",
  ),
...
#figure(
  image("s3/miiii/grads_norms_miiii.svg"),
...
Lastly, as can be seen in , when dropout was disabled, the model's performance diverged on the validation set (overfitting), even with other kinds of regularization (multi-task, l2, etc.). shows the accuracy through training for $cal(T)_"masked"$. The masking of $q in {2,3,5,7}$, does _not_ delay grokking on the remaining tasks notably, the spiking after generalization to easy tasks ($q in {11, 13, 17, 19}$) remains.
